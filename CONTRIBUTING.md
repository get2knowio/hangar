...
This guide covers everything you need to **run, configure, develop, and deploy** Hangar.
For *what Hangar is and why* — the rationale, the check catalog, and the `.hangar.json`
override — start with the [README](README.md). For the governing principles that reviews
enforce, read the [constitution](.specify/memory/constitution.md); the working distillation
...
- [Choosing an access mode](#choosing-an-access-mode)
- [Persistence: SQLite default, Postgres upgrade](#persistence-sqlite-default-postgres-upgrade)
- [Tests & CI](#tests--ci)
...
The backend is strictly layered — `api → services → {domain, providers, persistence}` — and
`domain` knows nothing about any platform. The **provider seam** (`providers/base.py`,
the `RepoProvider` Protocol) is the *only* place platform-specific strings live; adding a
provider is a new adapter module plus one registry entry, and the core gets no new branch.
See [`CLAUDE.md`](CLAUDE.md) for the full module map and the invariants that keep it honest.
...
- A **GitHub App** (App id + private key + webhook secret) installed on the org/user you
  ...
  self-hosted Gitea instance (see [Gitea setup](#gitea-setup)). At least one provider connection.
...
([`specs/001-fleet-control-plane/contracts/openapi.yaml`](specs/001-fleet-control-plane/contracts/openapi.yaml))
is the boundary between backend and frontend. Never hand-write frontend types — run
`npm run gen:api`. After changing any SQLAlchemy model, generate a migration with
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml). **Every defect fixed in review lands
with a regression test** (constitution): the required suites cover the `RepoProvider`
contract, remediation idempotency / PR-not-push, forward-auth modes, check evaluation,
cross-connection isolation, and the conditional-request / 304 path.

---

## Spec-driven workflow

This repo uses [Spec Kit](https://github.com/github/spec-kit). Feature design lives under
[`specs/001-fleet-control-plane/`](specs/001-fleet-control-plane/); the `/speckit-*` commands
drive constitution → spec → plan → tasks → implement, and git auto-commit hooks are
configured in `.specify/extensions.yml`. Before writing code, read:
...
- The [OpenAPI](specs/001-fleet-control-plane/contracts/openapi.yaml) and
  [UI](specs/001-fleet-control-plane/contracts/ui-spec.md) contracts
- The [constitution](.specify/memory/constitution.md) — its principles are **gates**, not
  aspirations — and the working distillation in [`CLAUDE.md`](CLAUDE.md).
